<template>
  <view class="wrapper">
    ...
    <view class="showcase2" v-if="showcase2.length > 0">
      <view
        class="showcase2-item"
        v-for="(item, i) in showcase2"
        :key="i"
        @click="goTheme(item.activityId)"
      >
    ...
    <view class="load-toast" v-if="showLoadTost" @click="closeLoadToast">
      <image
        class="load-toast-img"
        :src="loadMinapp.pic"
        mode=""
        @click="goLoad"
      ></image>
      <image
        class="load-toast-close"
...
export default {
  ...
  methods: {
    ...
    getGoodsJdList(type = null, lastGoodId = null) {
      this.$api
        .goodsJdList(this.p, this.nowOpt, lastGoodId)
        .then((res) => {
          if (type == "refresh") {
...
.card-view .button {
  ...
  right: 24rpx;
  top: 10rpx;
  animation: button-data-v-7cfb5c9e 0.4s;
